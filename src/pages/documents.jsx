          ...
          {documents.map((doc) => (
            <Card key={doc.id} className="shadow-lg border-none bg-white/5 backdrop-blur-md">
              <CardContent className="p-6">
                  ...
                  <div className="flex gap-2">
                    {!doc.analyzed && (
                      <Button
                        ...
                        className="bg-purple-600 hover:bg-purple-700 flex items-center gap-2"
                      >
                        {analyzing === doc.id ? <span className="animate-spin">⏳</span> : <Search className="h-4 w-4" />}
                        {analyzing === doc.id ? "Sending..." : "Analyze"}
                      </Button>
                    )}
                ...
                {doc.analyzed && doc.analysis && (
                  <div className="mt-4 space-y-2 bg-gray-900 p-4 rounded-md">
                    <h4 className="font-semibold text-white">Analysis Summary:</h4>
                    <p className="text-white/80">{doc.analysis.analysis_summary || "No summary available"}</p>

                    <h4 className="font-semibold text-white mt-2">Document Metadata:</h4>
                    <pre className="text-white/80 bg-gray-800 p-2 rounded">
                      {JSON.stringify(doc.analysis.document_metadata, null, 2)}
                    </pre>

                    <h4 className="font-semibold text-white mt-2">Entities Extracted:</h4>
                    <pre className="text-white/80 bg-gray-800 p-2 rounded">
                      {JSON.stringify(doc.analysis.entities_extracted, null, 2)}
                    </pre>

                    <h4 className="font-semibold text-white mt-2">Detected Issues:</h4>
                    <pre className="text-white/80 bg-gray-800 p-2 rounded">
                      {JSON.stringify(doc.analysis.issues_detected, null, 2)}
                    </pre>

                    <p className="text-white/80 mt-2">Confidence Score: {doc.analysis.confidence_score}</p>
